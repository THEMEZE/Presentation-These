    ...
    %\begin{column}{0.5\textwidth}
      ...
      \begin{columns}[c, totalwidth=\textwidth]
        ...
        \begin{column}{0.5\textwidth} 
          ...
          {\fontsize{6}{5}\selectfont 
          \begin{tikzpicture}[remember picture, overlay]
            \node [right]  at  (3,2.5)   (part)                            {$q_0(x;t) = n(x;t)$};
            \node [right]     (moment) [below=1mm of part]             {$q_1(x;t) = nu(x;t)$};
            \node [right]     (nrj) [below=1mm of moment]             {$q_2(x;t) = \varepsilon(x;t)$};
	          \draw[  opacity = 0.5 ]
              ...
              (moment) edge[line width=0.2ex ,color = red ,arrows = {Computer Modern Rightarrow[sharp ,length=0.2cm]-},  , out = 10 , in = 180  ]node[pos=1, right]{\color{red} \fontsize{6}{1}\selectfont \bfseries  local velocity maen }($(moment) + (2cm,0.5cm)$)
              (nrj) edge[line width=0.2ex ,color = black ,arrows = {Computer Modern Rightarrow[sharp ,length=0.2cm]-},  , out = -50 , in = 180  ]node[pos=1, right]{\color{black} \fontsize{6}{1}\selectfont \bfseries  local energie density }($(nrj) + (1cm,-1.25cm)$)
	          ;
          \end{tikzpicture}
          }
        %\end{block}
