      ...
      %\begin{block}{Eigenstates}
        ...
        \begin{tikzpicture}[overlay]
	        \draw[  opacity = 0.5 ]
	          ...
	          (Dephase) edge[line width=0.4ex ,color = blue ,arrows = -{Computer Modern Rightarrow[sharp ,length=0.2cm]},  , out = 90 , in = 180  ]node[pos=1, right]{\color{blue} \fontsize{5}{10}\selectfont \bfseries  $ \displaystyle \Delta = \hbar \frac{d \Phi }{dp} $ }($(Dephase) + (1cm,0.75cm)$)
	        ;
        \end{tikzpicture}
      %\end{block}
      \vspace{0.8cm}
      %\begin{block}{Eigenstates}
        $$ \left \{ \begin{array}{rcl} \displaystyle \frac{m}{\hbar} \theta_1 L + \Phi( \theta_1 - \theta_2) & = & 2 \pi I_1 \\ \displaystyle \frac{m}{\hbar} \theta_2 L + \Phi( \theta_2 - \theta_1) & = & 2 \pi I_2  \end{array}\right .  \quad I_1 , I_2 \in \mathbb{Z} + \frac{1}{2} $$
      %\end{block}
